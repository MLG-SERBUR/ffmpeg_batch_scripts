...
SET output=%~nx1_deint_sub_greyborder.mp4
...
ffmpeg ^
	...
	-c:v libx264 ^
	-crf 20 ^
	-filter_complex "[0:v]yadif=1:-1[sub];[sub]split=2[c][g];[g]hue=s=0[g];[c][g]blend=all_expr='if(lte(X,20)+gte(X,W-20)+lte(Y,20)+gte(Y,H-20),B,A)'[vid];[vid][0:s:0]overlay" ^
	-movflags +faststart ^
	-preset veryslow ^
